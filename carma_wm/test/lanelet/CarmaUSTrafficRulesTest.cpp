...
TEST(CarmaUSTrafficRules, carmaUSTrafficRules)
{
  ...
  std::shared_ptr<DirectionOfTravel> dot_oneway(new DirectionOfTravel(lanelet::utils::getId(), {ll_1, ll_2, ll_3}, DirectionOfTravel::OneWay, {lanelet::Participants::Vehicle}));
  ...
  std::shared_ptr<DirectionOfTravel> dot_bi(new DirectionOfTravel(lanelet::utils::getId(), {ll_4}, DirectionOfTravel::BiDirectional, {lanelet::Participants::Vehicle}));
  ll_4.addRegulatoryElement(dot_bi);
  // 7. create non-passable control line on left of first lanelet
  std::shared_ptr<PassingControlLine> pcl_left_1(new PassingControlLine(lanelet::utils::getId(), {ll_1.leftBound()}, {}, {}));
  ll_1.addRegulatoryElement(pcl_left_1);

  // 8. create passable control line from either side on first lanelet
  std::shared_ptr<PassingControlLine> pcl_right_1(new PassingControlLine(lanelet::utils::getId(), {ll_1.rightBound()}, {lanelet::Participants::Vehicle}, {lanelet::Participants::Vehicle}));
  ...
  std::shared_ptr<PassingControlLine> pcl_left_2(new PassingControlLine(lanelet::utils::getId(), {ll_3.leftBound(), ll_4.leftBound()}, {}, {}));
  std::shared_ptr<PassingControlLine> pcl_right_2(new PassingControlLine(lanelet::utils::getId(), {ll_2.rightBound(), ll_3.rightBound(), ll_4.rightBound()}, {}, {}));
  ...
  std::shared_ptr<PassingControlLine> pcl_area(new PassingControlLine(lanelet::utils::getId(), {area.outerBound()[1]}, {lanelet::Participants::Vehicle}, {lanelet::Participants::Vehicle}));
  ...
  ASSERT_EQ(25_mph, traffic_rules->speedLimit(ll_1).speedLimit);
  ASSERT_EQ(25_mph, traffic_rules->speedLimit(ll_2).speedLimit);
  ASSERT_EQ(25_mph, traffic_rules->speedLimit(ll_3).speedLimit);
  ASSERT_EQ(25_mph, traffic_rules->speedLimit(ll_4).speedLimit);

  // SpeedLimitInformation speedLimit(const ConstArea& area) const override;
  ASSERT_EQ(20_mph, traffic_rules->speedLimit(area).speedLimit);
